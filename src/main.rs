...
use individproj2::lib; // Adjust the import path to match your project name

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let matches = App::new("Movie Database Manager")
        .version("1.0")
        .author("Your Name")
        .about("A CLI application for managing movie data with CRUD operations")
        .subcommand(
            SubCommand::with_name("create-table")
                .about("Create the database table")
        )
        .subcommand(
            SubCommand::with_name("load-csv")
                .about("Load data from a CSV file into the database")
                .arg(Arg::with_name("csv-file").required(true))
        )
        .subcommand(
            SubCommand::with_name("query-top5")
                .about("Query and display the top 5 rows from the database")
        )
        .subcommand(
            SubCommand::with_name("query-best-genre")
                .about("Query and display the best movies of a genre")
                .arg(Arg::with_name("genre").required(true))
        )
        .get_matches();
    ...
    match matches.subcommand() {
        ...
        ("load-csv", Some(sub_m)) => {
            if let Some(csv_file) = sub_m.value_of("csv-file") {
                lib::load_csv_into_db(csv_file)?;
            }
        }
        ("query-top5", Some(_)) => {
            lib::query_top5()?;
        }
        ("query-best-genre", Some(sub_m)) => {
            if let Some(genre) = sub_m.value_of("genre") {
                lib::query_best_genre(genre)?;
            }
        }
        _ => {
            eprintln!("Invalid subcommand or arguments. Use --help for usage information.");
        }
    }
